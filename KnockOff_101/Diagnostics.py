...
_MMD_score = lambda P1,P2,kernel_type : _kernel_Z1Z2(P1,metric_type=kernel_type) + _kernel_Z1Z2(P2,metric_type=kernel_type) - 2*_kernel_Z1Z2(P1,P2,metric_type=kernel_type)
...
def MMD_checkQuality(X1st,X1st_knockoff,X2nd,X2nd_knockoff,n_partialSwap=20,set_seed=None,kernel_type='rbf'):
    ...
    score = [_MMD_score(LHS,fullSwap_RHS,kernel_type)]
    random.seed(set_seed)
    np.random.seed(set_seed)
    for _ in range(n_partialSwap):
        score += [_MMD_score(LHS,partialSwap(),kernel_type)]
